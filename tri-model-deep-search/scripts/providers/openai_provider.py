...
class OpenAIProvider(BaseProvider):
    ...
    async def expand_queries(self, topic: str, objective: str) -> list[str]:
        """
        Use ChatGPT to expand the initial query into related queries.
...
请只输出查询词列表，每行一个查询词，不要有编号或其他格式。"""
    ...
    async def execute_search(
        ...
        """
        Execute Stage 3: ChatGPT analysis of search results.
        ...
        try:
            ...
            self.logger.error(f"ChatGPT execution failed: {e}")
            return StageResult(
                provider_name=self.name,
                success=False,
                error_message=str(e)
